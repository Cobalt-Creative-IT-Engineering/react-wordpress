// ============================================================
//  WordPress Headless Client  –  REST API + ACF
//  Remplacez WP_BASE_URL par votre domaine WordPress
// ============================================================

export const WP_BASE_URL =
  import.meta.env.VITE_WP_URL ?? "https://votre-wordpress.com";
...
function buildParams(params: QueryParams): URLSearchParams {
  ...
  if (params.categories?.length)
    p.set("categories", params.categories.join(","));
  ...
  if (params.embed !== false) p.set("_embed", "1"); // embed par défaut
  return p;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function parseImage(raw: any): WPImage | null {
  const img =
    raw?._embedded?.["wp:featuredmedia"]?.[0] ??
    raw?._embedded?.["wp:featuredmedia"];
  if (!img) return null;
  return {
    id: img.id,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function parseTerms(raw: any, taxonomy: string): WPTerm[] {
  const terms = raw?._embedded?.["wp:term"] ?? [];
  const group = terms.find((t: WPTerm[]) =>
    t?.[0]?.taxonomy === taxonomy
  );
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function parsePost(raw: any): WPPost {
  return {
    id: raw.id,
    slug: raw.slug,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function parsePage(raw: any): WPPage {
  return {
    id: raw.id,
    slug: raw.slug,
    title: raw.title?.rendered ?? "",
...
export async function getPosts(params: QueryParams = {}): Promise<{
  ...
  const url = `${API}/posts?${buildParams(params)}`;
  const res = await fetch(url);
...
export async function getCPT<T extends Record<string, unknown>>(
  ...
  const url = `${API}/${cptSlug}?${buildParams(params)}`;
  return fetcher<T[]>(url);
}

// ---------- ACF (plugin ACF to REST API requis) -------------

export interface ACFOptions {
  /** Champ ACF pour les options globales du site */
  [key: string]: unknown;
}
...
export async function getACFForCPT(
  cptSlug: string,
  postId: number
...
// ---------- Menus (plugin WP REST API Menus requis) ---------
